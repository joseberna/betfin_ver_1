...
const renderStepsErrors = function (errors) {
    ...
    for (const file of Object.keys(errorsByFile)) {
        ...
        const errorMessages = errorsByFile[file]
            ...
            const beforeLines = before.map((line) => (0, chalk_1.default) `${line}\n`).join('');
            const afterLines = after.map((line) => (0, chalk_1.default) `${line}\n`).join('');
    ...
    console.error((0, chalk_1.default) `{red.bold Validation failed}\n\n`);
    console.error(messages.join('\n'));
    console.error((0, chalk_1.default) `🚨  {bold.red Migration unsuccessful}`);
};
exports.default = renderStepsErrors;
//# sourceMappingURL=steps-errors.js.map