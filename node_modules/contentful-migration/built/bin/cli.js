...
exports.runMigration = exports.createMakeRequest = exports.UrlBaseType = void 0;
...
var UrlBaseType;
(function (UrlBaseType) {
    UrlBaseType["Space"] = "space";
    UrlBaseType["Org"] = "org";
})(UrlBaseType = exports.UrlBaseType || (exports.UrlBaseType = {}));
const createMakeRequest = (client, { spaceId, environmentId, organizationId, limit = 10, host = 'api.contentful.com' }) => {
    const throttle = (0, p_throttle_1.default)({
        limit,
        interval: 1000,
        strict: false
    });
    const makeOrgBaseUrl = (url) => {
        const parts = [`https://${host}`, 'organizations', organizationId, (0, trim_1.default)(url, '/')];
        return parts.filter((x) => x !== '').join('/');
    };
    const makeSpaceBaseUrl = (url) => {
        const parts = [
            `https://${host}`,
            'spaces',
            spaceId,
    ...
    return function makeRequest(requestConfig) {
        const { url, baseType } = requestConfig, config = __rest(requestConfig, ["url", "baseType"]);
