...
module.exports = async function createSpaceStep(guideContext) {
    ...
    log(separator(GUIDE_MAX_WIDTH));
    log();
    wrappedLog(`${stepCount > 1 ? 'Next' : 'First'}, we’ll create a Space which is a container for all of your structured content. We’ll create and name the space '${highlightStyle(activeGuide.name)}' using the command: ${codeStyle(shellescape(createSpaceCommand))}`, GUIDE_MAX_WIDTH);
    log();
